...
location we wan them stored. We're the =load-path= ourselves and avoid calling =(package-initilize)=
(for performance reasons) so we need to set =package--init-file-ensured= to tell =package.el= to
not automatically call it on our behalf. Additionally we're setting =package-enable-at-startup= so
that packages will not automatically be loaded for us since =use-package= will be handling that.
...
project."
 ...
 (defun bmacs/show-hide-popup-buffer (&optional buffer-rx)
   (interactive)
   (let ((existing-popup-buffers
           (if buffer-rx
             (seq-filter
               (lambda (buff)
             ...
             (seq-filter
               (lambda (buff)
                 (seq-some
                   ...
                   bmacs-popups))
               (buffer-list)))))
   (cond ((= 1 (length existing-popup-buffers))
          (delete-window (get-buffer-window (car existing-popup-buffers))))
         ((> 0 (length existing-popup-buffers))
           (ivy-read "Open popup: "
                 (mapcar #'buffer-name popup-buffers)
                 :action (lambda (x)
                           (delete-window (get-buffer-window x)))
                 :caller 'bmacs/show-hide-popup-buffer))
         ((seq-empty-p popup-buffers)
          (message "No popup buffers found"))
         ((= 1 (length popup-buffers))
          (pop-to-buffer (car popup-buffers)))
         (t
          (ivy-read "Open popup: "
                 (mapcar #'buffer-name popup-buffers)
                 :action (lambda (x)
