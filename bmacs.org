...
(declare-function mu4e-follow-link "mu4e~view-browse-url-from-binding")
  ...
  (save-excursion
    (save-restriction
      ...
      (let (beg end candidates)
        ...
        ;; Normal Mode
        (setq end
              (if (eq (get-text-property (point) 'mu4e-url) 'highlight)
                  (point)
                (text-property-any
                 (point) (point-max) 'mu4e-url nil)))

        (while (setq beg (text-property-not-all
                          end (point-max) 'mu4e-url nil))
          (goto-char beg)
          (setq end (text-property-any
                     (point) (point-max) 'mu4e-url nil))
          (unless end
...
- [ ] Cleanup mu4e ace link support and create pr to ace-link
